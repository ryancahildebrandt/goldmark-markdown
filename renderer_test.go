...
func TestConvertedOutput(t *testing.T) {
	testCases := []struct {
		name     string
...
`,
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			// check rendered equivalence output
			assert := assert.New(t)
			buf := bytes.Buffer{}
			reader := text.NewReader([]byte(tc.source))
			parsed := NewTestMarkdown().Parser().Parse(reader)
			renderer := NewRenderer(tc.options...)
			err := renderer.Render(&buf, reader.Source(), parsed)
			...
			// check for rendering errors on all nodes
			ast.Walk(parsed, func(n ast.Node, entering bool) (ast.WalkStatus, error) {
				err := renderer.Render(io.Discard, reader.Source(), n)
				assert.NoError(err)
